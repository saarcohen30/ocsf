...
def parse_args():
    parser = argparse.ArgumentParser("Online Coalitional Skill Vector Games - TuneOptOCSF")
    ...
    parser.add_argument('-nb', "--n_budget_sizes", type=int, default=7)
    parser.add_argument('-bd', "--baseline_budgets", nargs="+", default=[50, 100, 150, 250, 500, 1000, 1500], help="Baseline budgets")
...
for l in range(n_budget_sizes):  
    #Initialize:
    if n_budget_sizes == 1:
        filename = 'TuneOptOCSF.pckl'
        filename_short = 'TuneOptOCSF-short.pckl'
        f = open('model.pckl', 'rb')
    elif arglist.multiplier:
        # filename = 'TuneOptOCSF-' + str(baseline_budgets[l]) + '.pckl'
        filename_short = 'TuneOptOCSF-short-' + str(baseline_budgets[l]) + '-mul-' + str(m) + '.pckl'
        ...
        # filename = 'TuneOptOCSF-' + str(baseline_budgets[l]) + '.pckl'
        filename_short = 'TuneOptOCSF-short-' + str(baseline_budgets[l]) + '-' + str(m) + '.pckl'
    ...
    CONSTRAINT = [[[0.0001 for q in range(k)] for j in range(alphas[i])] for i in range(m)]
    tunable_parameters = [[np.random.random(k) * 2e-11 for j in range(alphas[i])] for i in range(m)]
    CONSTRAINT = [[[CONSTRAINT[i][j][q] - tunable_parameters[i][j][q] for q in range(k)] for j in range(alphas[i])] for i in range(m)]
...
plt.title(title)
plt.show()
distPlot.savefig('TuneOptOCSF - Distance.pdf', bbox_inches = 'tight')
...
plt.title(title)
plt.show()
samplePlot.savefig('TuneOptOCSF - Sample Complexity.pdf', bbox_inches = 'tight')
...
#     if arglist.multiplier:
#         title = 'TuneOptOCSF-' + str(baseline_budgets[l])
#     else:
#         title = 'TuneOptOCSF-' + str(baseline_budgets[l])
...
plt.title(title)
plt.show()
regretPlot.savefig('TuneOptOCSF - Regret.pdf', bbox_inches = 'tight')
...
plt.title(title)
plt.show()
consPlot.savefig('TuneOptOCSF - Regret of Constraint Violations.pdf', bbox_inches = 'tight')