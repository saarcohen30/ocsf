...
for l in range(n_budget_sizes):  
    if n_budget_sizes == 1:
        filename = 'gamma-first-' + str(EPS) + '.pckl'
        f = open('model.pckl', 'rb')
    elif arglist.multiplier:
        filename = 'gamma-first-' + str(EPS) + '-' + str(baseline_budgets[l]) + '-mul-' + str(m) + '.pckl'
        ...
        filename = 'gamma-first-' + str(EPS) + '-' + str(baseline_budgets[l]) + '-' + str(m) + '.pckl'
    ...
    for sim in range(NUMBER_SIMULATIONS):  
        ...
        while len([q for q in range(1, k + 1) if len(coalition_structure[sim][q-1]) == budgets[q-1]]) < k:            
            ...
            if t < gamma * budgets_sum:                
                greedy_coalition = random.choice(plausible_actions)                
            else:
                ...
                for q in plausible_actions:
                    ...
                    if assign:
                        greedy_coalition = q+1
                        break
                    
            if greedy_coalition != 0:
                coalition_structure[sim][greedy_coalition-1].append(s)
                
                for i in range(m):
                    skill_coverage[sim][greedy_coalition-1][i][s[i]] += 1
                            
            # if t % 100 == 0:
            #     # filename = '\u03B3-first' + str(EPS) + '.pckl'
            #     f = open(filename, 'ab')
...
plt.title(title)
plt.show()
distPlot.savefig('\u03B3-first - ' + str(EPS) + ' - Distance.pdf', bbox_inches = 'tight')
...
plt.title(title)
plt.show()
samplePlot.savefig('\u03B3-first - ' + str(EPS) + ' - Sample Complexity.pdf', bbox_inches = 'tight')